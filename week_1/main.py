...
while running:
    ...
    for i in range(len(positions)):
     # 1. Update velocity with gravity, then update position
     ...
     if distance > max_distance and distance > 0:
         ...
         v_normal_mag = np.dot(velocities[i], normal)
         if v_normal_mag > 0:
             v_normal = v_normal_mag * normal
             v_tangent = velocities[i] - v_normal
             velocities[i] = v_tangent - WALL_RESTITUTION * v_normal
   ...
   #for two balls
   #dist_btw_balls=positions[0]-positions[1]
   # dx=dist_btw_balls[0]
   # dy=dist_btw_balls[1]
   # dist=(dx**2 + dy**2) ** 0.5
   # if dist<2*PARTICLE_RADIUS:
        #normal_vec=dist_btw_balls/dist

        
       # rel_vel=velocities[1]-velocities[0]
        #if np.dot(rel_vel,normal_vec)>0:

           #  overlap = 2 * PARTICLE_RADIUS - dist
           #  positions[0] += normal_vec * (overlap / 2) 
           #  positions[1] -= normal_vec * (overlap / 2) 

           #  v_norm1=np.dot(velocities[0],normal_vec)
           #  v_norm2=np.dot(velocities[1],normal_vec)
           #  v_along1=velocities[0]-v_norm1*normal_vec
           #  v_along2=velocities[1]-v_norm2*normal_vec
           #  velocities[0]=v_along1+v_norm2*normal_vec
           #  velocities[1]=v_along2+v_norm1*normal_vec

     #MULTIPLE BALLS        
     
    for i in range(NUM_PARTICLES):
        for j in range(i + 1,NUM_PARTICLES):
             dist_btw_balls = positions[i] - positions[j]
             dx = dist_btw_balls[0]
             dy = dist_btw_balls[1]
             dist = (dx**2 + dy**2) ** 0.5

             if dist < 2 * PARTICLE_RADIUS and dist > 0:
                 normal_vec = dist_btw_balls / dist
                 ...
                 if np.dot(rel_vel, normal_vec) > 0:
                   ...
                   overlap = 2 * PARTICLE_RADIUS - dist
                   positions[i] += normal_vec * (overlap / 2)
                   positions[j] -= normal_vec * (overlap / 2)

                   v_norm1 = np.dot(velocities[i], normal_vec)
                   v_norm2 = np.dot(velocities[j], normal_vec)

                   v_along1 = velocities[i] - v_norm1 * normal_vec
                   v_along2 = velocities[j] - v_norm2 * normal_vec

                   velocities[i] = v_along1 + v_norm2 * normal_vec
                   velocities[j] = v_along2 + v_norm1 * normal_vec
