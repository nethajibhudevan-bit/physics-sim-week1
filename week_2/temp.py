...
class SandSim:
    ...
    def update(self) -> None:
        grid=self._types.copy()

        for y in range(self.height-1,-1,-1):
            cols=_rng.permutation(self.width)
            for x in cols:
                material=grid[y,x]
                ...
                if  y+1<self.height:
                    if self._types[y+1,x]==0:
                        self._types[y+1,x]=material
                        self._types[y,x]=0
                        continue
                    d=_rng.choice([-1,1])

                    if 0<=x+d<self.width and self._types[y+1,x+d]==0:
                        self._types[y+1,x+d]=material
                        self._types[y,x]=0
                        continue
                    if 0<=x-d<self.width and self._types[y+1,x-d]==0:
                        self._types[y+1,x-d]=material
                ...
                if material==2:
                    d=_rng.choice([-1,1])
                    if 0<=x+d<self.width and self._types[y,x+d]==0:
                        self._types[y, x + d] = material
